...
class ExperimentManager:
    ...
    def setup_experiment(self) -> Optional[Tuple[BaseAlgorithm, Dict[str, Any]]]:
        ...
        env_collect = self.create_envs(1, no_log=False)

        self._hyperparams = self._preprocess_action_noise(hyperparams, saved_hyperparams, env)

        if self.continue_training:
            model = self._load_pretrained_agent(self._hyperparams, env, env_collect)
    ...
    def _load_pretrained_agent(self, hyperparams: Dict[str, Any], env: VecEnv, env_collect: VecEnv) -> BaseAlgorithm:
        # Continue training
