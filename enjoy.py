...
def main():  # noqa: C901
    ...
    model = ALGOS[algo].load(model_path, env=env, env_collect=env, custom_objects=custom_objects, device=args.device, **kwargs)
    print ("done")
    ...
    try:
        ...
        for time in range(args.n_timesteps):
            ...
            #print (np.shape(obs))
            action += args.policy_noise*np.random.normal(size=(1, action_dim))
        ...
        with open(args.save_file + '.pkl', 'wb') as fp:
